...
namespace tag_utils
{
  ...
  void drawCube(std::vector<cv::Point3f> &cube, cv::Mat &frame, cv::Mat &cameraMatrix, cv::Mat &distortionCoefficients, cv::Mat &rotationMatrix, cv::Mat &translationMatrix, cv::Scalar &color, std::vector<visualization_msgs::Marker> &marker_arr, std::string frame_id);
